...
function runForked(scenarioFile) {
  _utilJs.check.string(scenarioFile);
  return new Promise((resolve, reject) => {
    const child = (0, _child_process.fork)(_path2['default'].join(_utilJs.root, 'index.js'), ['run', '--path', scenarioFile]);
    child.on('error', reject);
    child.on('message', result => {
      if (result.type === 'SCENARIO_RESULT') resolve(result.data);
    });
  });
}

function runGroup(scenarioFiles) {
